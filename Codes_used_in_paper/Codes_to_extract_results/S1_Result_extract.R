##------This R code extracts the results of simulation  scenario (S1)--------
#----------------------------------------------------------------------------
...
true_theta3 = read.csv(paste0(folder_path, 
        "Simulated_datasets/3condition/twosame_third_partially_same_morediff_theta_",
                              "p_",p,"_n_",n,"_condition_",3,".csv"),header = T)[,-1] #loading true precision matrices 
                                                                                      #under three conditions in simulation (S1)
  ...
  for(j in lambda1_unique){
    ...
    #----loading the three estimated precision matrices under three conditions---
        c1 = sp$sparse$load_npz(paste0(folder_path, 
         'Simulated_datasets/Results/Python_res/3condition/RFGL_twosame_third_partially_same_new_morediff_theta',
...
for(i in lambda2_unique){
  ...
  for(j in lambda1_unique){
    ...
    for(simn in c(1:sim)){
      #----loading the three estimated precision matrices under three conditions---
      c1 = sp$sparse$load_npz(paste0(folder_path, 
      'Simulated_datasets/Results/Python_res/3condition/RCFGL_twosame_third_partially_same_new_morediff_theta',
              'p_',as.character(p),'_n_',as.character(n),'_condition_',
              as.character(0),'_sim_',
              as.character(simn),'_condition_',as.character(0),
              '_lambda1_', as.character(format(round(lambda1, 2), nsmall = 1)),
...
for(i in lambda2_unique){
  ...
  for(j in lambda1_unique){
    ...
    for(simn in c(1:sim)){
      FGL = readRDS(paste0(folder_path, 
      "Simulated_datasets/Results/R_res/3condition/CFGL_twosame_third_partially_same_morediff_theta_",
                           "p_",p,"_n_",n,"_sim_",simn,
                           "_lamda1_",as.character(lambda1),
                           "_lamda2_", as.character(lambda2),".rdata"))
...
saveRDS(SSE, paste0(folder_path, "Simulated_datasets/Extracted_Results/S1_SSE_results.rdata"))
saveRDS(PPV, paste0(folder_path, "Simulated_datasets/Extracted_Results/S1_PPV_results.rdata"))
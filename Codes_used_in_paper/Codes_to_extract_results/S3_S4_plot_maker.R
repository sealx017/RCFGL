##------This R code creates the SSE and PPV plots of the main paper for scenarios S3 and S4--------
#----------------------------------------------------------------------------------------------
...
PPV = readRDS(paste0(folder_path,"Simulated_datasets/Extracted_Results/S3_PPV_results.rdata"))
SSE = cbind(SSE, "(S3)"); PPV = cbind(PPV, "(S3)")
#-------Loading the SSE and PPV files of scenario S4----------
SSE2 = readRDS(paste0(folder_path,"Simulated_datasets/Extracted_Results/S4_SSE_results.rdata"))
PPV2 = readRDS(paste0(folder_path,"Simulated_datasets/Extracted_Results/S4_PPV_results.rdata"))
SSE2 = cbind(SSE2, "(S4)"); PPV2 = cbind(PPV2, "(S4)")
...
SSE3 = rbind(SSE, SSE2); SSE3$lambda2 = as.factor(SSE3$lambda2)
PPV3 = rbind(PPV, PPV2); PPV3$lambda2 = as.factor(PPV3$lambda2)

lam2.labs <- c("lambda2 = 0.05", "lambda2 = 0.1", "lambda2 = 0.2")
names(lam2.labs)<-levels(PPV3$lambda2)
color_codes = c("dodgerblue1","orangered1", "mediumorchid1")

#------Plotting SSE---------------
require(ggplot2)
p = 500; n = 100
png(paste0(folder_path, "Plots/S3_S4_SSE_",
           "p_",p,"_n_",n,".png"),
...
#------Plotting PPV---------------
png(paste0(folder_path, "Plots/S3_S4_PPV_",
           "p_",p,"_n_",n,".png"),
    height = 1200, width = 1400,res = 220)

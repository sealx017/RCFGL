##------This R code extracts the results of simulation  scenario (S4)--------
#----------------------------------------------------------------------------
...
true_theta1 = read.csv(paste0(folder_path, "Simulated_datasets/4condition/twosame_morediff_theta_",
                              "p_",p,"_n_",n,"_condition_",1,".csv"),header = T)[,-1]

true_theta2 = read.csv(paste0(folder_path, "Simulated_datasets/4condition/twosame_morediff_theta_",
                              "p_",p,"_n_",n,"_condition_",2,".csv"),header = T)[,-1]

true_theta3 = read.csv(paste0(folder_path, "Simulated_datasets/4condition/twosame_morediff_theta_",
                              "p_",p,"_n_",n,"_condition_",3,".csv"),header = T)[,-1]

true_theta4 = read.csv(paste0(folder_path, "Simulated_datasets/4condition/twosame_morediff_theta_",
...
for(i in lambda2_unique){
  ...
  for(j in lambda1_unique){
    ...
    for(simn in c(1:sim)){
      ...
      'Simulated_datasets/Results/Python_res/4condition/RFGL_twosame_morediff_theta_',
...
for(i in lambda2_unique){
  ...
  for(j in lambda1_unique){
    ...
    for(simn in c(1:sim)){
      #----loading the four estimated precision matrices under three conditions---
      c1 = sp$sparse$load_npz(paste0(folder_path, 
      'Simulated_datasets/Results/Python_res/4condition/RCFGL_twosame_morediff_theta_',
                                     'p_',as.character(p),'_n_',as.character(n),'_condition_',
                                     as.character(0),'_sim_',
                                     as.character(simn),'_condition_',as.character(0),
                                     '_lambda1_', as.character(format(round(lambda1, 2), nsmall = 1)),
...
names(PPV_lambda2) = c("RCFGL", "RCFGL","lambda1","lambda2")
Precision = tidyr::gather(PPV_lambda2[,c(1,3,4)],Method, Precision,
                          1, factor_key=TRUE)
Recall = tidyr::gather(PPV_lambda2[,c(2,3,4)],Method, Recall,
                       1, factor_key=TRUE)
...
for(i in lambda2_unique){
  ...
  for(j in lambda1_unique){
    ...
    for(simn in c(1:sim)){
      ...
      "Simulated_datasets/Results/R_res/4condition/FGL_twosame_morediff_theta_",
...
saveRDS(SSE, paste0(folder_path, "Simulated_datasets/Extracted_Results/S4_SSE_results.rdata"))
saveRDS(PPV, paste0(folder_path, "Simulated_datasets/Extracted_Results/S4_PPV_results.rdata"))
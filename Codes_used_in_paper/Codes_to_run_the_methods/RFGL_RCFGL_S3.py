...
#---------Code to run RFGL and RCFGL on simulated datasets under scenario S3-------------
...
for sim in range(10):
  ...
  for j in range(33): 
    ...
    B = pd.read_csv(folder_path + 'Simulated_datasets/4condition/two_twosame_morediff'+
                    'p_'+str(p)+'_n_'+str(n)+'_condition_'+str(1)+'_sim_'+str(sim)+'.csv');

    H = pd.read_csv(folder_path + 'Simulated_datasets/4condition/two_twosame_morediff'+
                    'p_'+str(p)+'_n_'+str(n)+'_condition_'+str(2)+'_sim_'+str(sim)+'.csv');

    M = pd.read_csv(folder_path + 'Simulated_datasets/4condition/two_twosame_morediff'+
                    'p_'+str(p)+'_n_'+str(n)+'_condition_'+str(3)+'_sim_'+str(sim)+'.csv');

    R = pd.read_csv(folder_path + 'Simulated_datasets/4condition/two_twosame_morediff'+
    ...
    for k in range(4):
     save_npz(folder_path+'Simulated_datasets/Results/Python_res/4condition/RFGL_two_twosame_morediff_theta'+
...
for sim in range(10):
  ...
  for j in range(24): 
    ...
    for k in range(4):
     save_npz(folder_path+'Simulated_datasets/Results/Python_res/4condition/RCFGL_two_twosame_morediff_theta'+
                  'p_'+str(p)+'_n_'+str(n)+'_condition_'+str(k)+'_sim_'+
                  str(sim)+'_condition_'+str(k)+'_lambda1_'+str(lambda1)+

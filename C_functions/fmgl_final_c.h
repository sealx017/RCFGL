...
void fused_lasso2_c(double* v, double* z,double L, double R)
{
    if (z[0] > z[1] + 2*L)
    {
        v[0] = z[0] - L;
...
*/
void fused_lasso_c(double* output, double* input, const size_t width, const double lambda, const double mu) {
...
void fmgl_subfusedLasso_nc(double* output, double* input,  const double* fx, const double* fy, double lam, double rho, const int N, const int K, const size_t Num)
{
    size_t N2 = N*N;
    ...
    for(size_t i = 0; i<Num; i++)
    {
        ...
        if(K==2)
            fused_lasso2_c(sarrayo, sarrayi, rho, lam);
        else
            fused_lasso_c(sarrayo, sarrayi, K, rho, lam);
        for(int k = 0; k<K;k++)
        {
...
void fmgl_subfusedLasso_diagonal_c(double* output, double* input,  const double* fx, const double* fy, double lam, double rho, const int N, const int K, const size_t Num)
{
    size_t N2 = N*N;
    double* sarrayi = new double[K];
    double* sarrayo = new double[K];
